(() => {
  ...
  const maps = {
    gastric: {
      ...
      stages: [
        { step: '1', title: '根除性手術後', subtitle: '先確認是否符合條文分期與手術條件', lanes: [
          { label: 'Adjuvant', criterion: '特定 Stage II–III 胃癌，根除性手術後', items: ['gc-s1-adj'] }
        ]},
        { step: '2', title: '局部晚期不可切除 / 復發 / 轉移：第一線', subtitle: '先看 HER2，再看 PD-L1 / CLDN18.2；同時保留化療 backbone', lanes: [
        ...
        { step: '3', title: '後線', subtitle: '健保條文中的後線與一般化療選項', lanes: [
    ...
    hcc: {
      ...
      stages: [
        { step: '0', title: '先過共同門檻', subtitle: 'Advanced HCC reimbursement gate', lanes: [
          { label: 'Eligibility', criterion: 'Child-Pugh A，且符合肝外轉移 / 大血管侵犯 / 條文定義 TACE failure 等條件之一', items: [] }
        ]},
        { step: '1', title: '第一線：四選一', subtitle: '依病人狀況選擇；此圖顯示的是健保可申請路徑，不做療效排序', lanes: [
        ...
        { step: '2', title: 'Sorafenib failure 後', subtitle: '這個 predecessor 是健保條文的關鍵，不應只標成「2L」', lanes: [
    ...
    aml: {
      ...
      stages: [
        ...
        { step: '2', title: 'Relapsed / refractory', subtitle: '目前主要 curated molecular route', lanes: [
          { label: 'FLT3-mutated R/R AML', criterion: '成人、計畫 HSCT；移植前使用', items: ['aml-gilteritinib-pre-hsct'] }
        ]},
        { step: '3', title: 'Maintenance / transplant 後', subtitle: '依 response、HSCT eligibility 與 MRD 狀態分流', lanes: [
  ...
  toolbar.hidden = true;
  toolbar.innerHTML = `<div class="clinical-view-label"><strong>檢視方式</strong><span>臨床路徑為健保給付導航，不是 treatment guideline。</span></div><div class="view-switch" role="group" aria-label="切換檢視方式"><button type="button" data-clinical-view="map">臨床路徑</button><button type="button" data-clinical-view="list">完整條文</button></div>`;
  ...
  function itemButton(itemId) {
    ...
    return `<button class="pathway-drug" type="button" data-map-item="${esc(itemId)}"><strong>${esc(x.drug)}</strong><small>${esc(x.regimen)}</small><div class="pathway-drug-meta">${bio}<span>§ ${esc(x.section)}</span>${x.prior_auth ? '<span>事審</span>' : ''}</div></button>`;
  }
  function renderLane(lane) {
    const items = (lane.items || []).map(itemButton).join('');
    return `<article class="pathway-lane${items ? '' : ' info-only'}"><div class="pathway-lane-head"><h4>${esc(lane.label)}</h4><p>${esc(lane.criterion || '')}</p></div>${items ? `<div class="pathway-drugs">${items}</div>` : '<div class="pathway-gate">符合後再進入下一步 ↓</div>'}</article>`;
  }
  function renderMap(cancerId) {
    const cfg = maps[cancerId];
    if (!cfg) return;
    panel.innerHTML = `<div class="clinical-map-intro"><div><p class="eyebrow">${esc(cfg.kicker)}</p><h3>${esc(cfg.title)}</h3><p>${esc(cfg.intro)}</p></div><span class="pathway-disclaimer">NHI reimbursement map</span></div><div class="pathway-stages">${cfg.stages.map(stage => `<section class="pathway-stage"><div class="pathway-stage-head"><span class="pathway-step">${esc(stage.step)}</span><div><h3>${esc(stage.title)}</h3><p>${esc(stage.subtitle || '')}</p></div></div><div class="pathway-lanes">${stage.lanes.map(renderLane).join('')}</div></section>`).join('')}</div><div class="pathway-alerts"><strong>給付限制提醒</strong><ul>${cfg.alerts.map(x => `<li>${esc(x)}</li>`).join('')}</ul></div>`;
  }
  function setMode(cancerId, mode) { if (!maps[cancerId]) return; modeByCancer[cancerId] = mode; sync(); }
  function sync() {
    const cancerId = currentCancer();
    const supported = Boolean(cancerId && maps[cancerId] && !resultsSection.hidden);
    toolbar.hidden = !supported;
    panel.hidden = true;
    if (!supported) { filterBar.hidden = false; activeFilters.hidden = false; results.hidden = false; return; }
    const mode = modeByCancer[cancerId] || 'map';
    toolbar.querySelectorAll('[data-clinical-view]').forEach(btn => { btn.classList.toggle('active', btn.dataset.clinicalView === mode); btn.setAttribute('aria-pressed', btn.dataset.clinicalView === mode ? 'true' : 'false'); });
    if (mode === 'map') { renderMap(cancerId); panel.hidden = false; filterBar.hidden = true; activeFilters.hidden = true; results.hidden = true; emptyState.hidden = true; }
    else { filterBar.hidden = false; activeFilters.hidden = false; results.hidden = false; }
  }
  toolbar.addEventListener('click', event => { const btn = event.target.closest('[data-clinical-view]'); if (btn) setMode(currentCancer(), btn.dataset.clinicalView); });
  panel.addEventListener('click', event => { const btn = event.target.closest('[data-map-item]'); if (!btn) return; const detailButton = [...document.querySelectorAll('[data-detail]')].find(x => x.dataset.detail === btn.dataset.mapItem); if (detailButton) detailButton.click(); });
  globalSearch?.addEventListener('input', event => { const cancerId = currentCancer(); if (maps[cancerId] && event.target.value.trim()) { modeByCancer[cancerId] = 'list'; setTimeout(sync, 0); } });
