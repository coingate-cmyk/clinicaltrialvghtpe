...
AUDIT_MARKER = 'v4.3.4-20260820-explicit-line-audit'
...
if VERSION not in s and 'const TRIAL_CLASSIFICATION_REPAIRS' not in s:
...
# 1b) Extend the curated table with additional records whose cancer + line are explicit
# in the title/cohort wording. These are the high-confidence findings from the full seed audit.
if AUDIT_MARKER not in s:
    old_tail = r"""    // KEYMAKER 06D: previously treated gastric/GEJ/EAC, explicitly 2L.
    'MK-3475-06D': [
        { type: '胃癌', lines: ['二線'] }
    ]
};
"""
    new_tail = r"""    // KEYMAKER 06D: previously treated gastric/GEJ/EAC, explicitly 2L.
    'MK-3475-06D': [
        { type: '胃癌', lines: ['二線'] }
    ],
...
"""
    if old_tail not in s:
        raise SystemExit('Could not find classification repair map tail')
